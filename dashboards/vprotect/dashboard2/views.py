...
def apiProxy(request):
    ...
    path = VPROTECT_API_URL + vprotectPath + queryParamSeparator + "project-uuid=" + request.user.tenant_id + queryParamSeparator + "user-uuid=" + request.user.id + queryParamSeparator + "user-name=" + request.user.username
    ...
    if is_endpoint_contentable(path):
        ...
        response2['3rd_party'] = 'HORIZON'
        response2['3rd_party_project'] = request.user.tenant_id
        return response2
        ...
        jsonResponse['3rd_party'] = 'HORIZON'
        jsonResponse['3rd_party_project'] = request.user.tenant_id
        return jsonResponse
    else:
